  ...
  return (
    <Panel className="p-5">
      ...
      <ul className="mt-4 max-h-85 space-y-3 overflow-y-auto pr-1">
        {groups.length === 0 && <Empty>{t("No groups yet")}</Empty>}
        {groups.map((group) => (
    ...
    <li className="flex items-center gap-3 rounded-xl bg-muted/60 px-3 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-muted-foreground">
          {t("Absent:")} <b className="text-foreground">{group.absent}</b> | {t("Late:")}{" "}
      ...
      <Link href="/progressbook" aria-label={t("Journal")}>
        <ClipboardList className="size-5 text-primary" />
      </Link>
      <Link href="/groups" aria-label={t("Open group")}>
