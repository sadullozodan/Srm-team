...
export default function EditExpensePage() {
  ...
  const { data, isPending, isError } = useQuery({ queryKey: queryKeys.detail("Expenses", id), queryFn: () => expensesApi.get(id), enabled: !!id });
  return (
    <div className="space-y-6">
      ...
      {isError ? <Card><CardContent className="p-6 text-sm text-destructive">Couldn&apos;t load.</CardContent></Card>
        ...
        : <ExpenseForm expenseId={id} initial={data} />}
    </div>
  );
}
