...
export default function EditSalaryPage() {
  ...
  const { data, isPending, isError } = useQuery({ queryKey: queryKeys.detail("Salaries", id), queryFn: () => salariesApi.get(id), enabled: !!id });
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Edit salary</h1>
      </div>
      {isError ? <Card><CardContent className="p-6 text-sm text-destructive">Couldn&apos;t load.</CardContent></Card>
        ...
        : <SalaryForm salaryId={id} initial={data} />}
    </div>
  );
}
