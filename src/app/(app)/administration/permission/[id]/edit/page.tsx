...
export default function EditPermissionPage() {
  ...
  const { data, isPending, isError } = useQuery({
    queryKey: queryKeys.detail("Permissions", id),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Edit permission</h1>
      </div>
      {isError ? (
        <Card><CardContent className="p-6 text-sm text-destructive">Couldn&apos;t load this permission.</CardContent></Card>
      ) : isPending ? (
        <div className="flex items-center justify-center py-16"><Loader2 className="size-6 animate-spin text-primary" /></div>
      ) : (
        <PermissionForm permissionId={id} initial={data} />
      )}
    </div>
  );
}
